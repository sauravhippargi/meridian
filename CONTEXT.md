...
### Where things stand right now (updated again, later same night)
- The first `seed:generate` attempt (Anthropic) crashed at 76% (ticket 730/956) with **"credit balance too low"** — confirmed **zero rows landed** in Postgres (generate-data.ts writes everything in ONE transaction at the very end; a mid-run crash means nothing persists, not a partial write). ~$0.82 was actually charged for the successful calls before the crash, unrecoverable.
- Tried **xAI (Grok)** as a fallback — key authenticates, but the team account hit its credit/spending limit on the very first dry-run call (before any real run started). Wired in as `GEN_PROVIDER=xai`, `grok-3-mini` confirmed as a working model name if that account's billing gets fixed later (check console.x.ai — error distinguished "used all credits" vs. "monthly spending limit reached", worth checking which).
- Switched to **Groq** (`GEN_PROVIDER=groq`). First model tried, `llama-3.3-70b-versatile`, doesn't support Groq's `json_schema` structured-output mode that `generateObject` needs — real technical limitation, not billing. Switched default to **`openai/gpt-oss-120b`**, which does support it. **Dry-run quality gate PASSED** ($0.01/17 samples) — cross-theme coverage, correct planted deals (Retool/Airtable/Attio), theme-appropriate severity. One cosmetic note: one transcript sample had literal "Interviewer:/Carlos:" dialogue formatting instead of pure first-person monologue — not blocking, worth a glance in later samples.
- `.env.local`'s `GEN_PROVIDER` is now `groq`. All 4 provider keys (Anthropic, xAI, Google, Groq) are present in `.env.local` if a future session wants to switch again — check which one actually has usable credit before assuming any of them works; three separate billing/limit surprises happened in one evening.
- **`npm run seed:generate` has NOT yet been re-run with Groq as of this update** — the dry-run gate just passed; running the full ~1,033-call generation is the very next action.
- Everything else written this session (extraction pipeline, all 4 query functions, competitor mapping) is committed to `main` on GitHub and does NOT depend on any particular session staying alive — safe to pick up from a fresh environment. The provider-switching code (multi-provider `lib/queries/../scripts/seed/llm.ts` support for anthropic/google/openai/xai/groq) is NOT yet committed as of this update — do that before switching environments again.

### The sequential plan from here
1. **Confirm whether `seed:generate` is still running or needs a restart.** Check `ps aux | grep generate-data`; if gone, verify Postgres is still empty (`SELECT count(*) FROM raw_tickets`) and re-run `npm run seed:generate`.
2. **Once it completes:** verify counts (~956 tickets / ~63 transcripts / ~14 deals in Postgres), spot-check a handful for quality (same bar as the dry-run gate already passed).
3. **Set up the Trigger.dev Cloud project** (account, `TRIGGER_PROJECT_REF`, `TRIGGER_SECRET_KEY` into `.env.local`) — a dashboard/auth task, needs a human. Currently the single biggest blocker to running extraction and the agent for real. Can happen in parallel with steps 1-2 (no code dependency).
4. **Run extraction** (`trigger/extract-mentions.ts` via `npx trigger.dev@latest dev`, or call `lib/extraction/pipeline.ts`'s functions directly from a script first to unblock faster if the Trigger.dev dev server setup is slow) → populates ClickHouse `mentions` (~5,000 rows expected).
5. **Verify the 4 query functions against real data** — run `listOpportunitiesRanked`, `getThemeEvidence`, `getImpactProjection`, `getThemeVolumeStats`/`getSignalSummary`/`getThemeTrends` and actually look at the output. **The scoring formula (§5 Phase A3 item 3) was implemented but explicitly deferred for review against real numbers — do that review now**, before treating any `build_now`/`build_next`/`deprioritize` call as final. Check it reproduces the demo narrative (§1): usage-billing #1, multi-entity hidden gem, dunning correctly deprioritized despite volume.
6. **Build `createAgentStream()`** (`lib/agent-stream.ts`) and **`chat.agent()`** (`trigger/agent.ts`) against the now-verified query functions. Hybrid orchestration per §2's decision (scripted main flow, LLM-driven follow-ups). Honor the per-chapter event order in §3.
7. **End-to-end test**: ask "what should we prioritize next quarter?" against the live agent, confirm all chapters stream, all three wow-moments land, provenance drill-down works.
8. **Phase A4** (bonus prize): `trigger/sync-oltp-to-olap.ts`, `docs/architecture.md` with a Mermaid diagram, query optimization pass.
9. **Phase A5** (deploy + submit): re-verify Vercel deploy, `npx trigger.dev deploy`, flip `NEXT_PUBLIC_AGENT_MODE=live`, finalize `SUBMISSION.md` with real numbers, fix `README.md` (currently 2 lines), record the demo video (opens with live product, max 5 min), add MIT LICENSE, make repo public, submit early (morning of July 23, not midnight AoE).
